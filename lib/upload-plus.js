...
const replaceFilenameInUrl = (url, oldName, newName) => {
  if (!url) return url;
  const encodedOldName = encodeURIComponent(oldName);
  const encodedNewName = encodeURIComponent(newName);
  return url.replace(encodedOldName, encodedNewName);
};
  ...
  try {
    ...
    if (pluginSettings.random_filename) {
      ...
      if (finalFilePath !== newPath) {
        ...
        if (storedFile.url) {
          storedFile.url = replaceFilenameInUrl(storedFile.url, originalName, newName);
        }
      }
    }
