...
const generateRandomFilename = (originalName) => {
  ...
  const hash = crypto.createHash('sha256').update(timestamp + originalName).digest('hex');
  const ext = path.extname(originalName);
  return hash + ext;
};

const replaceFilenameInPath = (filePath, newName) => {
  const dirname = path.dirname(filePath);
  return path.join(dirname, newName).replace(/\\/g, '/');
};
...
const processStoredFile = async (hookData) => {
  try {
    ...
    let finalFilePath = storedFile.path;

    // 0. Случайное переименование файла (если включено)
    if (pluginSettings.random_filename) {
      const originalName = storedFile.name;
      const newName = generateRandomFilename(originalName);
      const oldBasename = path.basename(storedFile.path);
      const newPath = replaceFilenameInPath(finalFilePath, newName);

      if (finalFilePath !== newPath) {
        await moveFileToNewPath(finalFilePath, newPath);
        finalFilePath = newPath;
        storedFile.path = newPath;
        storedFile.name = newName;
        if (storedFile.url) {
          const newUrl = storedFile.url.replace(oldBasename, newName);
          if (newUrl !== storedFile.url) {
            storedFile.url = newUrl;
          }
        }
      }
    }
